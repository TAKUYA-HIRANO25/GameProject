...
///
/// 概要:
/// - 行列・ベクトルの生成や変換、行列演算、正規化など、ゲームで頻繁に使用する数学関数を提供。
/// - MakeAffineMatrix/MakePerspectiveFovMatrix/Inverseなど、3D レンダリング向けの関数群を含む。
///
/// 注意:
/// - 関数の多くは行列やベクトルの乗算・逆行列計算を行うため、数値安定性に注意して使用。
/// - パフォーマンスが重要な箇所では呼び出し回数やコピー回数を最適化。
/// </summary>
namespace MyMath {
