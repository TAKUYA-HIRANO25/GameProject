...
/// - レンダーターゲット/デプスステンシルバッファ、デスクリプタヒープなど共通リソースを生成。
/// - シェーダーコンパイル(DXC)やImGui の初期化、フレーム前後の処理ラッパーを提供。
/// - テクスチャ用のアロケータや一時転送リソースを管理。
/// 
/// 使用上の注意:
/// - 初期化はアプリケーションの起動時に一度だけ行い、終了時に後片付けを行うこと。
/// - スレッドセーフではなく、レンダリングスレッド(通常はメインスレッド)で操作。
/// - リソース作成系メソッド(CreateBufferResource, CreateTextureResourece等)を通して
///   GPUリソースを一元管理することでメモリ管理を簡素化。
/// </summary>
